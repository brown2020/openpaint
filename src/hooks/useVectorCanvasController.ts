...
export function useVectorCanvasController() {
  ...
  useEffect(() => {
    if (activeTool !== "pen") {
      penToolRef.current.cancelPath();
      return;
    }

    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key !== "Enter" && e.key !== "Escape") return;
      if (!penToolRef.current.isDrawing()) return;
      e.preventDefault();
      if (e.key === "Escape") {
        penToolRef.current.cancelPath();
      } else {
        penToolRef.current.finishPath(false);
      }
